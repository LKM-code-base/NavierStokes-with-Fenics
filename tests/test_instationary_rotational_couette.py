...
class InstatRotationalCouetteFlow(InstationaryProblem):
    def __init__(self, n_points, n_refinements, radii, main_dir=None):
        super().__init__(main_dir, start_time=0.0, end_time=20.0,
                         desired_start_time_step=0.0025, n_max_steps=5)

        # parameters for mesh
        self._radii = radii
        self._n_points = n_points
        self._n_refinements = n_refinements
        self._problem_name = "InstatRotationalCouette"

        # dimensionless parameters and starting values of Omega and Alpha = dOmega/dt
        # Alpha is considered to be constant until t_acceleration, after that it is 0
        # Note that _acceleration_time has not yet been implemented in ns_bdf_solver; for now it is considered to be 1.0
        omega_start = 0
        self._alpha_start = 1
        self._acceleration_time = 1.0
        self.set_parameters(Re=550.0, Ro=1.0, Omega=dlfn.Constant(omega_start), Alpha=dlfn.Constant(self._alpha_start))
        ...
        velocity_exact_x = "- x[1] * omega * (ri * pow(ro,2) / (pow(ri,2) - pow(ro,2)) * (ri / (pow(x[0],2) + pow(x[1],2)) - 1 / ri) - 1)"
        velocity_exact_y = "omega * x[0] * (ri * pow(ro,2) / (pow(ri,2) - pow(ro,2)) * (ri / (pow(x[0],2) + pow(x[1],2)) - 1 / ri) - 1)"
        self._velocity_exact = dlfn.Expression([velocity_exact_x, velocity_exact_y], degree=2, omega=omega_end, ri=ri, ro=ro)
    ...
    def setup_mesh(self):
        # create mesh
        self._mesh, self._boundary_markers = spherical_shell(2, self._radii, self._n_points, self._n_refinements)
...
def test_instat_rotational_couette():
    instat_rotational_couette_flow = InstatRotationalCouetteFlow(60, 0, (0.25, 0.5))
